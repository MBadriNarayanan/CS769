...
from torch.nn import Dropout, Linear, Sequential, Softmax
...
class BertSentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        self.classifier = Sequential(
            Linear(config.hidden_size, config.num_labels), Softmax(dim=-1)
        )
    ...
    def forward(self, input_ids, attention_mask):
        ...
        x = self.classifier(output["pooler_output"])
        x = self.dropout(x)
        scores = F.logsigmoid(x)
        return scores
