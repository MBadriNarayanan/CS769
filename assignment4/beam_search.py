...
def beam_search(model, src_hiddens, beam_size=5, decode_max_length=10, to_word=True):
    ...
    dec_input = torch.LongTensor(batch_size, beam_size).fill_(
        bos_id
    )  # [batch_size, hidden_size]
    dec_hidden = torch.zeros(
    ...
    for t in range(decode_max_length):
        ...
        log_prob, dec_hidden = model.decode_one_step(
            t, dec_input, dec_hidden, src_hiddens
        )
        ...
        if eos_mask is not None and eos_mask.sum() > 0:
            log_prob.masked_scatter_(
                eos_mask.unsqueeze(2), pad_vec.expand((eos_mask.sum(), vocab_size))
            )
        ...
        dec_hidden = select_hiddens_by_beams(
            dec_hidden, top_beamid
        )  # [batch, beam_size, hidden_size]
...
def topK(score):
    ...
    top_values, top_indices = torch.topk(score, k=beam_size, dim=1)
    top_beamid = (top_indices // vocab_size).view(batch_size, beam_size)
    top_wordid = (top_indices % vocab_size).view(batch_size, beam_size)
    top_score = top_values.view(batch_size, beam_size)
...
def select_hiddens_by_beams(hiddens, beam_id):
    ...
    Example:
        ...
        new_hiddens = torch.FloatTensor([[[0.1627, 0.4812, 0.1167, 0.4318],
                                        ...
                                        [0.9372, 0.4993, 0.5471, 0.9169]]])
    """
    new_hiddens = torch.zeros(hiddens.size())
    batch_size, _ = beam_id.size()
    for i in range(batch_size):
        new_hiddens[i] = torch.index_select(hiddens[i], 0, beam_id[i])
    return new_hiddens
...
def extract_sequences(top_score, top_wordids, top_beamids):
    ...
    sort_score = torch.zeros((batch_size, beam_size), dtype=torch.float)

    for i in range(batch_size):
        scores, indices = torch.sort(top_score[i], descending=True)
        for j in range(beam_size):
            beam_index = indices[j].item()
            word_index = top_wordids[-1][i][beam_index].item()

            for step in range(len(top_wordids) - 1, -1, -1):
                sequence[i, j, step] = word_index
                sort_score[i, j] = scores[j]

                beam_index = top_beamids[step][i][beam_index].item()
                word_index = (
                    top_wordids[step - 1][i][beam_index].item()
                    if step > 0
                    else word_index
                )
